...
class Feed extends Component {
  ...
  finishEditHandler = (postData) => {
    ...
    const getGraphqlQuery = (imagePath) =>
      this.state.editPost
        ? {
            query: `
              mutation UpdateExistingPost($postId: ID!, $title: String!, $content: String!, $imageUrl: String!) {
                updatePost(postId: $postId, postData: {title: $title, content: $content, imageUrl: $imageUrl}) {
                  _id
                  title
                  content
                  imageUrl
    ...
    fetch(`http://localhost:8080/post-image`, {
      ...
      .then((resData) =>
        fetch(`http://localhost:8080/graphql`, {
          ...
          body: JSON.stringify(getGraphqlQuery(resData.filePath)),
        })
      )
      .then((res) => res.json())
      .then((resData) => {
